...
const Cardapio = (() => {
    ...
    function render() {
        ...
        if (!list.length) {
            grid.innerHTML = `<div class="empty-state" style="grid-column:1/-1;"><i class="fa-solid fa-book-open"></i>Nenhum doce ativo no cardápio ainda. Cadastre um produto para começar.</div>`;
            return;
        }
        ...
        grid.innerHTML = list.map(p => `
            <div class="menu-card js-menu-card" data-id="${p.id}">
                <div class="thumb">
                    ${p.imagemUrl ? `<img src="${p.imagemUrl}">` : '<i class="fa-solid fa-cookie-bite"></i>'}
                ...
                <div class="body">
                    <div class="cat">${Utils.escapeHtml(p.categoria || 'Doces')}</div>
