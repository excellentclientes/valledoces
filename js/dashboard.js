...
const Dashboard = (() => {
        ...
        el.innerHTML = `
            <div class="stat-grid">
                ...
                <div class="stat-card acc-3">
                    <div class="icon-badge"><i class="fa-solid fa-cookie-bite"></i></div>
                    <div class="label">Doces vendidos</div>
    ...
    function ordersOfDay(date) {
        return Store.pedidos.filter(p => p.status !== 'cancelado' && isSameDay(Utils.toDate(p.createdAt), date));
    }
    ...
    function renderTopProdutos() {
        ...
        if (!arr.length) { box.innerHTML = emptyState('fa-cookie-bite', 'Sem vendas registradas.'); return; }
        const max = arr[0][1];
