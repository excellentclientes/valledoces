...
const Pedidos = (() => {
    ...
    async function advanceStatus(id) {
        ...
        try {
            await window.db.collection('pedidos').doc(id).update({ status: novo });
            if (novo === 'entregue') {
                const snap = await window.db.collection('producao').where('pedidoId', '==', id).get();
                const batch = window.db.batch();
                snap.forEach(d => batch.update(d.ref, { status: 'concluido' }));
                await batch.commit();
            }
    ...
    function cancelPedido(id) {
        Utils.confirmDialog('Cancelar este pedido? O lançamento financeiro relacionado será removido.', async () => {
            try {
                const batch = window.db.batch();
    ...
    async function submitPedido(e) {
        ...
        try {
            ...
            batch.set(finRef, {
                tipo: 'receita', categoria: 'Venda de doces', descricao: `Pedido #${numero} — ${cliente.nome}`,
                valor: total, formaPagamento: data.formaPagamento, data: new Date(),
